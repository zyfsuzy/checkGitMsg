...
const escapeSpecialChars = (result, needReview) => {
  // eslint-disable-next-line no-useless-escape
  const specialChars = ['`'];

  let commitMessage = result;
  // eslint-disable-next-line array-callback-return
  specialChars.map((item) => {
    // If user types "feat: `string`", the commit preview should show "feat: `\string\`".
    // Don't worry. The git log will be "feat: `string`"
    commitMessage = result.replace(new RegExp(item, 'g'), '\\`');
  });
  return {
    commitMessage,
    needReview
...
module.exports = (answers, config) => {
  ...
  let Reviewers = wrap(answers.Reviewers, wrapOptions) || '';
  ...
  return escapeSpecialChars(result, needReview);
};
